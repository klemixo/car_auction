...
const store = new Vuex.Store({
                ...
                state.cars.forEach(car => {
                    if (state.filters.runMax.value) {
                        const mileAge = parseInt(
                            car["odometer"].replace(/\,/g, ""),
                            10
                        );
                        if (
                            mileAge > state.filters.runMin.value &&
                            mileAge < state.filters.runMax.value
                        ) {
                            newCars.push(car);
                        }
                    } else {
                    ...
                    if (
                        state.filters.auctionHouseCopart.value &&
                        !state.filters.auctionHouseIAAI.value
                    ) {
                        newCars = newCars.filter(
                            car => car.production_year % 2 !== 0
                        );
                    }
                    if (
                        state.filters.auctionHouseIAAI.value &&
    ...
    mutations: {
        ...
        GET_CARS(state) {
            ...
            for (const property in state.filters) {
                if (
                    ...
                    state.filters[property].value === false
                ) {
                    continue;
                }
                if (
                    state.filters[property].field === "site" &&
                    state.filters[property].value === true
                ) {
            ...
            axios
                .get(`https://vinfax.info/api/cars?${filtersString}`)
                .then(res => {
        ...
        GET_FILTERS(state, newMarka) {
            const reqUrl = newMarka
                ? `https://vinfax.info/api/filters?newMarka=${newMarka}`
                : `https://vinfax.info/api/filters`;
            axios
                .get(reqUrl)
                .then(res => {
